...
type Survey struct {
	ID string `bson:"_id,omitempty"` //auto generated by mongodb
...
// edit a survey in all fields
	router.PUT("/surveys/:token", func(c *gin.Context) {
		...
		_, err = surveysCollection.UpdateOne(context.TODO(), 
			bson.M{"token": token}, 
			bson.M{"$set": bson.M{"title": survey.Title, "questions": survey.Questions}}) //update title and question arr in the survey with that token
...
//edit a question
	router.PUT("/surveys/:token/:questionNo", func(c *gin.Context) {
		...
		_, err = surveysCollection.UpdateOne(context.TODO(),
			bson.M{"token": token}, 
			bson.M{"$set": bson.M{"questions": survey.Questions}}) //update the question arr which has one edited question
